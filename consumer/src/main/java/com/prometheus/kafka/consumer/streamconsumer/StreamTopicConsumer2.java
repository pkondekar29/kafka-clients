...
public class StreamTopicConsumer2 {
    ...
    public static void main(String[] args) {
        ...
        props.put("group.id", "stream-users-2");
        // https://kafka.apache.org/documentation.html#consumerconfigs

        KafkaConsumer<String, User> consumer = new KafkaConsumer<>(props);

        // Here we are subscribing to the topic
        consumer.subscribe(Arrays.asList("invalid-users"));
        try {
            while (true) {
                // Poll loop is started
                ConsumerRecords<String, User> consumerRecords = consumer.poll(Duration.ofMillis(10));
                consumerRecords.forEach(record -> {
                    System.out.println("Topic: " + record.topic() + ", Offset: " + record.offset() + ", Partition: " + record.partition() + ", Key: " + record.key() + ", Value: [Invalid-user]" + record.value());
                });
            }
        } finally {
            consumer.close();
